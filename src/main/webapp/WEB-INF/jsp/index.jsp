...
<ui:templete title="Welcome to Save to Drive Srttc Project">
                	...
                	<div class="col-xl-12" style="text-align:center">
	                    <c:choose>
	                        <c:when test='${empty(user)}'>
	                            <p style="font-size: 20px; color: red">You are not Connected. Please Connect with Google. </p>
	                            	<button id="auth-btn" class="btn btn-link btn-responsive" style="font-size: larger;" onclick="location.href = '${pageContext.request.contextPath}/api/oauth/google/redirect'">
	                                  Click here to Connect with Google
	                             </button>
	                        </c:when>
                        	<c:otherwise>	                         
                        		<p style="font-size: 20px; color: green">Hi, <c:out value="${user.name}" default="Guest" />. </p>   
	                            <p style="font-size: 20px; color: green; word-wrap:break-word">Congratulations! You are successfully Connected with Google
